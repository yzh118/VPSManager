...
function calculateYyscHash() {
    ...
    $yysc_path = $config['yysc_file'];
    
    // 如果文件不存在，返回空哈希
    if (!file_exists($yysc_path)) {
        logAction("警告: yysc.conf文件不存在");
    ...
    if ($content === false) {
        logAction("错误: 无法读取yysc.conf文件");
        return hash($config['hash_algorithm'], '');
    }
    
    // 计算哈希值
    $hash = hash($config['hash_algorithm'], $content);
    logAction("计算yysc.conf哈希值: $hash");
...
function validateAppId($app_id) {
    ...
    if (!file_exists($yysc_path)) {
        logAction("警告: yysc.conf文件不存在，无法验证应用ID");
        return false;
    }
    ...
    // 检查应用ID是否存在于yysc.conf中
    $pattern = '/ID=\<' . preg_quote($app_id, '/') . '\>/';
    if (preg_match($pattern, $content)) {
        logAction("应用ID验证成功: $app_id");
        return true;
    } else {
        logAction("应用ID验证失败: $app_id (不存在于yysc.conf中)");
        return false;
    }
}
