...
get_available_tags() {
    ...
    while IFS= read -r line; do
        ...
        case "$line" in
            ...
            "}")
                if [ -n "$tag_line" ]; then
                    IFS=' ' read -ra tag_arr <<< "$tag_line"
                    for t in "${tag_arr[@]}"; do
                        [ -z "$t" ] && t="未分类"
                        local found=false
                        for existing_tag in "${tags[@]}"; do
                            if [ "$existing_tag" = "$t" ]; then
                                found=true
                                break
                            fi
                        done
            ...
            case $choice in
                ...
                s|S)
                    ...
                    while [ "$search_tag_mode" = true ]; do
                        ...
                        echo "${CDB}可用标签列表:${NC}"
                        echo "-------------"
                        local available_tags=($(get_available_tags))
                        if [ ${#available_tags[@]} -eq 0 ]; then
                            echo "${CDR}当前没有可用的标签${NC}"
                        else
                            for ((i=0; i<${#available_tags[@]}; i++)); do
                                printf "%d) %s\n" $((i+1)) "${available_tags[$i]}"
                            done
                        fi
                        echo ""
                        
                        echo "请输入要搜索的标签 (留空显示所有应用，q返回):"
                        ...
                        if [ -n "$search_tag" ]; then
                            ...
                            clear
                            local search_output
                            search_output=$(show_app_list "$search_tag")
                            local search_market_name
                            search_market_name=$(echo "$search_output" | tail -n 1)
                            local search_app_list
                            search_app_list=$(echo "$search_output" | sed '$d')
                            
                            echo "$search_app_list"
                            echo ""
                            echo "-------------"
                            echo "|<q>${CDR}返回搜索${NC}"
                            echo "===================="
...
# NAT小鸡管理配置文件
NAT_VM_CONF="/usr/local/vpsmanager/nat_vms.conf"
...
get_vm_conf() {
    local vmname="$1"
    grep -E "^\[$vmname\]" -A 10 "$NAT_VM_CONF" 2>/dev/null | grep -v '^\[' | grep -v '^$'
}
set_vm_conf() {
    local vmname="$1"; shift
    # 先删除原有配置
    sed -i "/^\[$vmname\]/,/^\[/d" "$NAT_VM_CONF" 2>/dev/null
    # 追加新配置
    echo "[$vmname]" >> "$NAT_VM_CONF"
    for kv in "$@"; do
        echo "$kv" >> "$NAT_VM_CONF"
    done
}
...
nat_vm_menu() {
    ...
    if ! command -v virsh &>/dev/null; then
        echo "未检测到 virsh（KVM管理工具），无法管理NAT小鸡。"
        read -p "是否自动安装libvirt和KVM依赖？(Y/n): " yn
        yn=${yn,,}; yn=${yn:-y}
        if [[ $yn == y* ]]; then
            if command -v apt &>/dev/null; then
                apt update && apt install -y libvirt-clients libvirt-daemon-system qemu-kvm
                ...
                apk add libvirt qemu-img qemu-system-x86_64
                rc-service libvirtd start
            else
                echo "暂不支持你的系统，请手动安装libvirt和KVM相关依赖。"
                read -p "按回车键返回..."
                return
            fi
            echo "依赖安装完成，请重新进入NAT小鸡管理菜单。"
            read -p "按回车键返回..."
            return
        else
            echo "已取消安装，无法使用NAT小鸡管理功能。"
            read -p "按回车键返回..."
            return
        fi
    fi
    while true; do
        clear
        echo "===== NAT小鸡管理（KVM） ====="
        mapfile -t vmlist < <(virsh list --all | awk 'NR>2 && $2!="" {print $2}' | cat)
        if [ ${#vmlist[@]} -eq 0 ]; then
            echo "未检测到任何KVM虚拟机。"
            echo "输入 n 新建虚拟机，或 0 返回主菜单。"
            read -p "请输入: " idx
            ...
            if [[ "$idx" == "n" || "$idx" == "N" ]]; then
                ...
                if ! kvm-ok >/dev/null 2>&1; then
                    echo "${CDR}KVM加速不可用！请检查CPU虚拟化是否开启。${NC}"
                ...
                read -p "内存(MB): " mem
                read -p "磁盘大小(GB): " disk
                read -p "选择ISO镜像路径: " iso
                read -p "SSH端口: " sshport
                read -p "NAT端口（如1-5,7,9）: " natports
                imgpath="/var/lib/libvirt/images/${newname}.qcow2"
                qemu-img create -f qcow2 "$imgpath" ${disk}G
                
                # 3. 错误捕获和自动清理
                if ! virt-install --name "$newname" --ram "$mem" --vcpus "$vcpu" --disk path="$imgpath",format=qcow2 --cdrom "$iso" --network network=default,model=virtio --graphics vnc,listen=0.0.0.0 --noautoconsole; then
                    echo "${CDR}虚拟机创建失败！请检查ISO路径、权限或其它参数。${NC}"
                    rm -f "$imgpath" # 清理失败的磁盘镜像
                else
                    set_vm_conf "$newname" "ssh_port=$sshport" "nat_ports=$(parse_ports "$natports")"
                    echo "已创建虚拟机 $newname"
                fi
                read -p "按回车继续..."
                continue
            fi
            echo "无效输入！"; read -p "按回车键继续..."; continue
        fi
        echo "序号  虚拟机名           状态"
        for i in "${!vmlist[@]}"; do
            state=$(virsh domstate "${vmlist[$i]}")
            printf "<%d> %-18s %s\n" "$((i+1))" "${vmlist[$i]}" "$state"
        done
        echo "-------------"
        echo "0. 返回主菜单"
        echo "========================="
        read -p "请输入虚拟机序号(或n新建): " idx
        if [[ "$idx" == "0" ]]; then return; fi
        if [[ "$idx" == "n" || "$idx" == "N" ]]; then
            # 新建虚拟机
            clear
        ...
        if ! [[ "$idx" =~ ^[0-9]+$ ]] || (( idx < 1 || idx > ${#vmlist[@]} )); then
            echo "无效序号！"; read -p "按回车键继续..."; continue
        fi
        vmname="${vmlist[$((idx-1))]}"
        while true; do
            clear
            state=$(virsh domstate "$vmname")
            echo "===== NAT小鸡：$vmname 状态：$state ====="
            echo "1. 开机"
            echo "2. 关机"
            echo "3. 重启"
            echo "4. 检测运行状态"
            echo "5. 设置SSH端口"
            echo "6. 设置NAT端口（支持1-5,7,9等）"
            echo "7. 安装/重装系统"
            echo "8. 修改密码"
            echo "9. 设置限流/带宽"
            echo "10. 设置定时删除"
            ...
            case $op in
                1)
                    virsh start "$vmname" && echo "已开机" || echo "开机失败"; read -p "按回车继续...";;
                2)
                    virsh shutdown "$vmname" && echo "已关机" || echo "关机失败"; read -p "按回车继续...";;
                3)
                    virsh reboot "$vmname" && echo "已重启" || echo "重启失败"; read -p "按回车继续...";;
                4)
                    state=$(virsh domstate "$vmname"); echo "当前状态：$state"; read -p "按回车继续...";;
                5)
                    read -p "请输入SSH端口: " sshport
                    set_vm_conf "$vmname" "ssh_port=$sshport"
                    echo "已设置SSH端口为$sshport"; read -p "按回车继续...";;
                6)
                    read -p "请输入NAT端口（如1-5,7,9）: " natports
                    ports=$(parse_ports "$natports")
                    set_vm_conf "$vmname" "nat_ports=$ports"
                    echo "已设置NAT端口为: $ports"; read -p "按回车继续...";;
                7)
                    echo "===== 安装/重装系统 ====="
                    read -p "选择ISO镜像路径: " iso
                    read -p "磁盘大小(GB): " disk
                    imgpath="/var/lib/libvirt/images/${vmname}.qcow2"
                    virsh destroy "$vmname" 2>/dev/null
                    virsh undefine "$vmname" 2>/dev/null
                    qemu-img create -f qcow2 "$imgpath" ${disk}G
                    virt-install --name "$vmname" --ram 1024 --vcpus 1 --disk path="$imgpath",format=qcow2 --cdrom "$iso" --network network=default,model=virtio --graphics vnc,listen=0.0.0.0 --noautoconsole
                    echo "已重装系统 $vmname"; read -p "按回车继续...";;
                8)
                    echo "===== 修改密码 ====="
                    echo "如镜像支持cloud-init，可通过cloud-init或virt-customize修改。"
                    read -p "请输入新密码: " newpass
                    if command -v virt-customize &>/dev/null; then
                        imgpath="/var/lib/libvirt/images/${vmname}.qcow2"
                        virt-customize -a "$imgpath" --root-password password:"$newpass" && echo "密码已修改" || echo "修改失败"
                    else
                        echo "未检测到virt-customize，请手动修改或用cloud-init。"
                    fi
                    set_vm_conf "$vmname" "password=$newpass"
                    read -p "按回车继续...";;
                9)
                    echo "===== 设置限流/带宽 ====="
                    read -p "请输入最大带宽（Mbps，0为不限）: " bw
                    if [[ "$bw" =~ ^[0-9]+$ && "$bw" -gt 0 ]]; then
                        iface=$(virsh domiflist "$vmname" | awk '/network/ {print $5; exit}')
                        virsh domiftune "$vmname" "$iface" --outbound "$(($bw*1000*1000))"
                        set_vm_conf "$vmname" "bandwidth=${bw}Mbps"
                        echo "已设置最大带宽为${bw}Mbps"
                    else
                        echo "未设置带宽限制"
                    fi
                    read -p "按回车继续...";;
                10)
                    echo "===== 设置定时删除 ====="
                    read -p "请输入定时删除时间（如2024-06-30 23:59）: " deltime
                    atime=$(date -d "$deltime" +%s 2>/dev/null)
                    now=$(date +%s)
                    if [[ -n "$atime" && "$atime" -gt "$now" ]]; then
                        echo "virsh destroy $vmname; virsh undefine $vmname; rm -f /var/lib/libvirt/images/${vmname}.qcow2" | at "$deltime"
                        set_vm_conf "$vmname" "delete_at=$deltime"
                        echo "已设置定时删除：$deltime"
                    else
                        echo "时间格式错误或早于当前时间"
                    fi
                    read -p "按回车继续...";;
