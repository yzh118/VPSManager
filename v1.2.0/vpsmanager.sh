...
show_app_list_fast() {
    ...
    for ((i=0; i<app_count; i++)); do
        ...
        local found=false
        for ((j=0; j<${#tag_groups[@]}; j++)); do
            [ "${tag_groups[$j]}" = "$t" ] && found=true && break
        done
        [ "$found" = false ] && tag_groups+=("$t")
    done
    # 标签排序
    if [ -n "$tag_order" ]; then
        local ordered_tags=() remaining_tags=()
        IFS=' ' read -ra order_array <<< "$tag_order"
        for order_tag in "${order_array[@]}"; do
            for ((i=0; i<${#tag_groups[@]}; i++)); do
                [ "${tag_groups[$i]}" = "$order_tag" ] && ordered_tags+=("$order_tag") && break
            done
        done
        for t in "${tag_groups[@]}"; do
            local found=false
            for ot in "${ordered_tags[@]}"; do
                [ "$t" = "$ot" ] && found=true && break
            done
            [ "$found" = false ] && remaining_tags+=("$t")
        done
        tag_groups=("${ordered_tags[@]}" "${remaining_tags[@]}")
    fi
    # 分组输出
    for tag in "${tag_groups[@]}"; do
        printf "║ ---------%s---------\n" "$tag"
        local tag_app_ids=() tag_app_titles=() tag_app_texts=() tag_app_count=0
        for ((i=0; i<app_count; i++)); do
            local current_tag=${app_tags[$i]}
            [ -z "$current_tag" ] && current_tag="未分类"
            if [ "$current_tag" = "$tag" ]; then
                tag_app_ids[$tag_app_count]=${app_ids[$i]}
                tag_app_titles[$tag_app_count]=${app_titles[$i]}
                tag_app_texts[$tag_app_count]=${app_texts[$i]}
        ...
        for ((i=0; i<tag_app_count; i++)); do
            ...
            if should_show_likes; then
                # 直接获取最新数据
                likes=$(get_app_likes "$app_id" 2>/dev/null)
                if [ $? -eq 0 ] && [ -n "$likes" ]; then
                    update_likes_cache "$app_id" "$likes"
                else
                    likes="0"
...
app_market_menu() {
    ...
    while true; do
        ...
        if [ -f "$CACHE_FILE" ]; then
            ...
            local cache_age=$((current_time - last_cache_time))
            
            if [ -z "$cached_app_data" ] || [ "$cache_age" -gt 300 ]; then
                # 重新解析应用列表
                echo "${CDB}正在加载应用列表...${NC}"
                cached_app_data=$(show_app_list_fast 2>&1)
                last_cache_time=$current_time
                
                # 调试：检查解析结果
                if [ -z "$cached_app_data" ]; then
                    echo "${CDR}警告：应用列表解析结果为空${NC}" >&2
                fi
            fi
            
            # 显示应用列表（排除最后一行市场名称）
            local app_list_content
            app_list_content=$(echo "$cached_app_data" | sed '$d')
            echo "$app_list_content"
                    ...
                    case $like_choice in
                        ...
                        2)
                            ...
                            if [ -n "$http_code" ] && [ "$http_code" -eq 200 ]; then
                                ...
                                # 测试点赞数解析
                                echo "${CDB}正在测试点赞数解析...${NC}"
                                local test_likes=$(get_app_likes "1" 2>&1)
                                echo "${CDB}解析结果: $test_likes${NC}"
                            else
                        ...
                        3)
                            ...
                            ;;
                        4)
                            echo "${CDB}API自动获取过程:${NC}"
                            echo "-------------"
                            
                            # 检查独立配置文件
                            if [ -f "$LIKE_API_URL_FILE" ]; then
                                local saved_url=$(cat "$LIKE_API_URL_FILE")
                                echo "1. 独立配置文件: $saved_url"
                            else
                                echo "1. 独立配置文件: 不存在"
                            fi
                            ...
                            if [ -f "$CACHE_FILE" ]; then
                                ...
                                while IFS= read -r line; do
                                    ...
                                    if [[ "$line" =~ ^like_api=(.*)$ ]]; then
                                        ...
                                        echo "2. 缓存文件配置: $like_api_url"
                                        if [[ "$like_api_url" == *"example.com"* ]]; then
                                            echo "   (这是示例地址，将被忽略)"
                                        fi
                                        break
                                    fi
                                done < "$CACHE_FILE"
                                if [ -z "$like_api_url" ]; then
                                    echo "2. 缓存文件配置: 未找到"
                                fi
                            else
                                echo "2. 缓存文件配置: 缓存文件不存在"
                            fi
                            ...
                            if [ -f "$CACHE_FILE" ]; then
                                ...
                                while IFS= read -r line; do
                                    if [[ "$line" =~ ^Name=(.*)$ ]]; then
                                        market_name="${BASH_REMATCH[1]}"
                                        echo "3. 市场名称: $market_name"
                                        echo "   自动生成API: https://$market_name/like_api.php"
                                        break
                                    fi
                                done < "$CACHE_FILE"
                            ...
                            if [ -f "$MARKET_URL_FILE" ]; then
                                local market_url=$(cat "$MARKET_URL_FILE")
                                echo "4. 市场URL: $market_url"
                                local domain=$(echo "$market_url" | sed -E 's|^https?://([^/]+).*|\1|')
                                if [ -n "$domain" ]; then
                                    echo "   自动生成API: https://$domain/like_api.php"
                                fi
                            else
                                echo "4. 市场URL: 未找到"
                            fi
                            ...
                            echo "-------------"
                            echo "${CDB}最终选择的API: $(get_like_api_url)${NC}"
                            ;;
                        *)
                            echo "${CDR}无效选项${NC}"
                            ;;
...
get_like_api_url() {
    ...
    # 2. 从yysc.conf文件中读取like_api配置（排除示例地址）
    if [ -f "$CACHE_FILE" ]; then
        local like_api_url=""
        while IFS= read -r line; do
            line=$(echo "$line" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
            if [[ "$line" =~ ^like_api=(.*)$ ]]; then
                like_api_url="${BASH_REMATCH[1]}"
                # 排除示例地址
                if [[ "$like_api_url" != *"example.com"* ]] && [[ "$like_api_url" != *"api.example.com"* ]]; then
                    echo "$like_api_url"
                    return 0
                fi
                break
